...
public class Solver {
	...
	public static boolean solveIT(Grid grid, String pieceChoiceMethod) {
		...
		//first we create a pile with the frst possible piece and its orientation
		...
		while (!pile.isEmpty()) { // simulation of recursivity
			...
			currentPiece.getKey().setOrientation(currentPiece.getValue().getValue());//set the orientation
			grid.setPiece(currentPiece.getKey().getPosY(), currentPiece.getKey().getPosX(), currentPiece.getKey()); //update the grid
	 ...
	 * @param pile
	 * @param currentpiece
	 * @param lastPiece the last piece not connected
	 * @return
	 */
			...
			if( (currentpiece.getPosX() > lastPiece.getPosX() && currentpiece.getPosY() == lastPiece.getPosY())
				...
				if(currentpiece.getPosY() > lastPiece.getPosY()+1){
					Pair<Piece, Orientation> forty_two = pile.peek();
						while (forty_two.getKey().getPosX() > lastPiece.getPosX()+1 && forty_two.getKey().getPosY() > lastPiece.getPosY() +1)
						 {
					forty_two = pile.pop();
					}
						return pile;
